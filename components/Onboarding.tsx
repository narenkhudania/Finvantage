...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete, onBackToLanding }) => {
  ...
  const handleProceedIdentifier = async () => {
    ...
    try {
      const exists = await checkIdentifier(formData.identifier.trim());
  ...
  const handleSignup = async () => {
    if (formData.password.length < 8) { setError('Min 8 characters required.'); return; }
    ...
    try {
      ...
      onComplete({
        ...
        profile: {
          ...
          mobile:         isEmail ? '' : formData.identifier.trim(),
          dob:            profile.dob ?? '',
        ...
        <div className="max-w-md w-full mb-6 sm:mb-10 flex items-center justify-between px-6 relative">
          ...
          {stepConfig.map((s, i) => (
            <div key={i} className="flex flex-col items-center relative z-10">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-700 ${onboardingStep >= i ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white text-slate-400 border border-slate-200'}`}>
                <s.icon size={16} />
              </div>
              <span className={`text-[7px] font-black uppercase tracking-[0.1em] mt-2 transition-colors duration-500 ${onboardingStep >= i ? 'text-indigo-600' : 'text-slate-400'}`}>{s.title}</span>
            </div>
          ))}
        </div>
      )}
            ...
            <div className="space-y-8 animate-in fade-in zoom-in-95 duration-500">
              ...
              <div className="space-y-5">
                <div className="space-y-2.5">
                  ...
                  <input type="text" placeholder="ravindra@wealth.terminal" className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:ring-4 focus:ring-indigo-100 focus:border-indigo-600 outline-none font-bold text-base transition-all placeholder:text-slate-300" value={formData.identifier} onChange={e => setFormData({...formData, identifier: e.target.value})} onKeyDown={e => e.key === 'Enter' && handleProceedIdentifier()} />
                </div>
                ...
                <button onClick={handleProceedIdentifier} disabled={isProcessing} className="w-full py-3.5 bg-indigo-600 text-white rounded-xl font-black text-sm uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 group shadow-lg shadow-indigo-100 disabled:opacity-60">
          ...
          {authStep === 'signup' && (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
              ...
              <div className="space-y-4">
                ...
                <button onClick={handleSignup} disabled={isProcessing} className="w-full py-3.5 bg-indigo-600 text-white rounded-xl font-black text-sm uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 disabled:opacity-60">
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <input type="password" placeholder="••••••••" className="w-full pl-12 pr-5 py-4 bg-slate-50 border border-slate-200 rounded-xl font-bold text-base outline-none focus:border-indigo-600" value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} onKeyDown={e => e.key === 'Enter' && handleLogin()} />
                  </div>
                </div>
                <button onClick={handleLogin} disabled={isProcessing} className="w-full py-4 bg-indigo-600 text-white rounded-xl font-black text-sm uppercase tracking-widest hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-100 disabled:opacity-60">
                ...
                <div className="text-center space-y-8 animate-in zoom-in-95 duration-1000">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-slate-500 text-sm font-medium leading-relaxed">
                      Welcome, <span className="text-indigo-600 font-bold">{loggedInFirstName || formData.firstName}</span>. System readiness is nominal.
                    </p>
                  </div>
                  ...
                  <button onClick={handleFinishOnboarding} disabled={isProcessing} className="w-full py-4 bg-slate-900 text-white rounded-xl font-black text-sm uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl active:scale-95 flex items-center justify-center gap-2 group disabled:opacity-60">
